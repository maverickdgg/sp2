...
void Sp2_Scene3::Init()
{
    ...
    suit = Human("spacesuit", 30, 180, Vector3(150, 0, 130));
    ...
    ChestBurster = AlienEnemy("ChestBurster", 25, 0, Vector3(30, 0, 50));
...
void Sp2_Scene3::Update(double dt)
{
    ...
    if (b_isDisplayUI == true && collision(ChestBurster, player))
    ...
    if (collision(camera.position, collisionVec, player.boundary))
    {
        b_isWorn == false;
    }
}
...
void Sp2_Scene3::RenderBB8(BB8 x)
{
    modelStack.PushMatrix();
    modelStack.Translate(x.pos.x, x.pos.y, x.pos.z);
    modelStack.Rotate(x.viewAngle, 0, 1, 0);
...
{
    modelStack.PushMatrix();
    modelStack.Translate(ChestBurster.pos.x, ChestBurster.pos.y - 29.5, ChestBurster.pos.z);
